...
router.get("/", async (req, res) => {
  ...
  const result = await query(
    `select orderId, orderDate, totalAmount, shiptoAddress as address, shiptoCity as city, shiptoState as state, shiptoPostalCode as zip, shiptoCountry as country, customerId from ordersummary where customerId='${req.session.customerId}'`
  );
