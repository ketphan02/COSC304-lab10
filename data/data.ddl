drop table if exists incart;
drop table if exists orderproduct;
drop table if exists productinventory;
drop table if exists review;
drop table if exists product;
drop table if exists ordersummary;
drop table if exists category;
drop table if exists paymentmethod;
drop table if exists shipment;
drop table if exists warehouse;
drop table if exists customer;



CREATE TABLE customer (
    customerId          INT AUTO_INCREMENT,
    firstName           VARCHAR(40),
    lastName            VARCHAR(40),
    email               VARCHAR(50),
    phonenum            VARCHAR(20),
    address             VARCHAR(50),
    city                VARCHAR(40),
    state               VARCHAR(20),
    postalCode          VARCHAR(20),
    country             VARCHAR(40),
    userid              VARCHAR(20),
    password            VARCHAR(30),
    PRIMARY KEY (customerId)
);

CREATE TABLE paymentmethod (
    paymentMethodId     INT AUTO_INCREMENT,
    paymentType         VARCHAR(20),
    paymentNumber       VARCHAR(30),
    paymentExpiryDate   DATE,
    customerId          INT,
    PRIMARY KEY (paymentMethodId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid)
        ON UPDATE CASCADE ON DELETE CASCADE 
);

CREATE TABLE warehouse (
    warehouseId         INT AUTO_INCREMENT,
    warehouseName       VARCHAR(30),    
    PRIMARY KEY (warehouseId)
);

CREATE TABLE shipment (
    shipmentId          INT AUTO_INCREMENT,
    shipmentDate        DATETIME,   
    shipmentDesc        VARCHAR(100),   
    warehouseId         INT, 
    PRIMARY KEY (shipmentId),
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId) ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE ordersummary (
    orderId             INT AUTO_INCREMENT,
    orderDate           DATETIME,
    totalAmount         DECIMAL(10,2),
    shiptoAddress       VARCHAR(50),
    shiptoCity          VARCHAR(40),
    shiptoState         VARCHAR(20),
    shiptoPostalCode    VARCHAR(20),
    shiptoCountry       VARCHAR(40),
    customerId          INT,
	shipmentId          INT,
    PRIMARY KEY (orderId),
    FOREIGN KEY (customerId) REFERENCES customer(customerid) ON UPDATE CASCADE ON DELETE CASCADE,
	FOREIGN KEY (shipmentId) REFERENCES shipment(shipmentId) ON UPDATE CASCADE ON DELETE CASCADE 		
);

CREATE TABLE category (
    categoryId          INT AUTO_INCREMENT,
    categoryName        VARCHAR(50),    
    PRIMARY KEY (categoryId)
);

CREATE TABLE product (
    productId           INT AUTO_INCREMENT,
    productName         VARCHAR(40),
    productPrice        DECIMAL(10,2),
    productImageURL     VARCHAR(100),
    productImage        BLOB,
    productDesc         VARCHAR(1000),
    categoryId          INT,
    PRIMARY KEY (productId),
    FOREIGN KEY (categoryId) REFERENCES category(categoryId)
);

CREATE TABLE orderproduct (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE incart (
    orderId             INT,
    productId           INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (orderId, productId),
    FOREIGN KEY (orderId) REFERENCES ordersummary(orderId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE productinventory ( 
    productId           INT,
    warehouseId         INT,
    quantity            INT,
    price               DECIMAL(10,2),  
    PRIMARY KEY (productId, warehouseId),   
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE NO ACTION,
    FOREIGN KEY (warehouseId) REFERENCES warehouse(warehouseId)
        ON UPDATE CASCADE ON DELETE NO ACTION
);

CREATE TABLE review (
    reviewId            INT AUTO_INCREMENT,
    reviewRating        INT,
    reviewDate          DATETIME,   
    customerId          INT,
    productId           INT,
    reviewComment       VARCHAR(1000),          
    PRIMARY KEY (reviewId),
    FOREIGN KEY (customerId) REFERENCES customer(customerId)
        ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (productId) REFERENCES product(productId)
        ON UPDATE CASCADE ON DELETE CASCADE
);

INSERT INTO category(categoryName) VALUES ('Beverages');
SET @cat1 = LAST_INSERT_ID();
INSERT INTO category(categoryName) VALUES ('Condiments');
SET @cat2 = LAST_INSERT_ID();
INSERT INTO category(categoryName) VALUES ('Dairy Products');
SET @cat3 = LAST_INSERT_ID();
INSERT INTO category(categoryName) VALUES ('Produce');
SET @cat4 = LAST_INSERT_ID();
INSERT INTO category(categoryName) VALUES ('Meat/Poultry');
SET @cat5 = LAST_INSERT_ID();
INSERT INTO category(categoryName) VALUES ('Seafood');
SET @cat6 = LAST_INSERT_ID();
INSERT INTO category(categoryName) VALUES ('Confections');
SET @cat7 = LAST_INSERT_ID();
INSERT INTO category(categoryName) VALUES ('Grains/Cereals');
SET @cat8 = LAST_INSERT_ID();

INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Chai', @cat1, '10 boxes x 20 bags',18.00);
SET @prod1 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Chang',@cat1,'24 - 12 oz bottles',19.00);
SET @prod2 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Aniseed Syrup',@cat2,'12 - 550 ml bottles',10.00);
SET @prod3 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Chef Anton''s Cajun Seasoning',@cat2,'48 - 6 oz jars',22.00);
SET @prod4 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Chef Anton''s Gumbo Mix',@cat2,'36 boxes',21.35);
SET @prod5 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Grandma''s Boysenberry Spread',@cat2,'12 - 8 oz jars',25.00);
SET @prod6 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Uncle Bob''s Organic Dried Pears',@cat4,'12 - 1 lb pkgs.',30.00);
SET @prod7 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Northwoods Cranberry Sauce',@cat2,'12 - 12 oz jars',40.00);
SET @prod8 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Mishi Kobe Niku',@cat5,'18 - 500 g pkgs.',97.00);
SET @prod9 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Ikura',@cat6,'12 - 200 ml jars',31.00);
SET @prod10 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Queso Cabrales',@cat3,'1 kg pkg.',21.00);
SET @prod11 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Queso Manchego La Pastora',@cat3,'10 - 500 g pkgs.',38.00);
SET @prod12 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Tofu',@cat4,'40 - 100 g pkgs.',23.25);
SET @prod13 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Genen Shouyu',@cat2,'24 - 250 ml bottles',15.50);
SET @prod14 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Pavlova',@cat7,'32 - 500 g boxes',17.45);
SET @prod15 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Alice Mutton',@cat5,'20 - 1 kg tins',39.00);
SET @prod16 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Carnarvon Tigers',@cat6,'16 kg pkg.',62.50);
SET @prod17 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Teatime Chocolate Biscuits',@cat7,'10 boxes x 12 pieces',9.20);
SET @prod18 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Sir Rodney''s Marmalade',@cat7,'30 gift boxes',81.00);
SET @prod19 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Sir Rodney''s Scones',@cat7,'24 pkgs. x 4 pieces',10.00);
SET @prod20 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Gustaf''s Knackebread',@cat8,'24 - 500 g pkgs.',21.00);
SET @prod21 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Sasquatch Ale',@cat1,'24 - 12 oz bottles',14.00);
SET @prod22 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Steeleye Stout',@cat1,'24 - 12 oz bottles',18.00);
SET @prod23 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Inlagd Sill',@cat6,'24 - 250 g  jars',19.00);
SET @prod24 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Boston Crab Meat',@cat6,'24 - 4 oz tins',18.40);
SET @prod25 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Jack''s New England Clam Chowder',@cat6,'12 - 12 oz cans',9.65);
SET @prod26 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Singaporean Hokkien Fried Mee',@cat8,'32 - 1 kg pkgs.',14.00);
SET @prod27 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Louisiana Fiery Hot Pepper Sauce',@cat2,'32 - 8 oz bottles',21.05);
SET @prod28 = LAST_INSERT_ID();
INSERT product(productName, categoryId, productDesc, productPrice) VALUES ('Laughing Lumberjack Lager',@cat1,'24 - 12 oz bottles',14.00);
SET @prod29 = LAST_INSERT_ID();

INSERT INTO warehouse(warehouseName) VALUES ('Main warehouse');
SET @warehouse1 = LAST_INSERT_ID();
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod1, @warehouse1, 5, 18);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod2, @warehouse1, 10, 19);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod3, @warehouse1, 3, 10);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod4, @warehouse1, 2, 22);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod5, @warehouse1, 6, 21.35);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod6, @warehouse1, 3, 25);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod7, @warehouse1, 1, 30);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod8, @warehouse1, 0, 40);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod9, @warehouse1, 2, 97);
INSERT INTO productinventory(productId, warehouseId, quantity, price) VALUES (@prod10, @warehouse1, 3, 31);

INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Arnold', 'Anderson', 'a.anderson@gmail.com', '204-111-2222', '103 AnyWhere Street', 'Winnipeg', 'MB', 'R3X 45T', 'Canada', 'arnold' , 'test');
SET @cus1 = LAST_INSERT_ID();
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Bobby', 'Brown', 'bobby.brown@hotmail.ca', '572-342-8911', '222 Bush Avenue', 'Boston', 'MA', '22222', 'United States', 'bobby' , 'bobby');
SET @cus2 = LAST_INSERT_ID();
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Candace', 'Cole', 'cole@charity.org', '333-444-5555', '333 Central Crescent', 'Chicago', 'IL', '33333', 'United States', 'candace' , 'password');
SET @cus3 = LAST_INSERT_ID();
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Darren', 'Doe', 'oe@doe.com', '250-807-2222', '444 Dover Lane', 'Kelowna', 'BC', 'V1V 2X9', 'Canada', 'darren' , 'pw');
SET @cus4 = LAST_INSERT_ID();
INSERT INTO customer (firstName, lastName, email, phonenum, address, city, state, postalCode, country, userid, password) VALUES ('Elizabeth', 'Elliott', 'engel@uiowa.edu', '555-666-7777', '555 Everwood Street', 'Iowa City', 'IA', '52241', 'United States', 'beth' , 'test');
SET @cus5 = LAST_INSERT_ID();


INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (@cus1, '2019-10-15 10:25:55', 91.70);
SET @ord1 = LAST_INSERT_ID();
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord1, @prod1, 1, 18);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord1, @prod5, 2, 21.35);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord1, @prod10, 1, 31);

INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (@cus2, '2019-10-16 18:00:00', 106.75);
SET @ord2 = LAST_INSERT_ID();
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord2, @prod5, 5, 21.35);

INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (@cus3, '2019-10-15 3:30:22', 140);
SET @ord3 = LAST_INSERT_ID();
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord3, @prod6, 2, 25);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord3, @prod7, 3, 30);

INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (@cus2, '2019-10-17 05:45:11', 327.85);
SET @ord4 = LAST_INSERT_ID();
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord4, @prod3, 4, 10);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord4, @prod8, 3, 40);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord4, @prod13, 3, 23.25);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord4, @prod28, 2, 21.05);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord4, @prod29, 4, 14);

INSERT INTO ordersummary (customerId, orderDate, totalAmount) VALUES (@cus5, '2019-10-15 10:25:55', 277.40);
SET @ord5 = LAST_INSERT_ID();
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord5, @prod5, 4, 21.35);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord5, @prod19, 2, 81);
INSERT INTO orderproduct (orderId, productId, quantity, price) VALUES (@ord5, @prod20, 3, 10);

-- New SQL DDL for lab 8
UPDATE product SET productImageURL = 'img/1.jpg' WHERE ProductId = @prod1;
UPDATE product SET productImageURL = 'img/2.jpg' WHERE ProductId = @prod2;
UPDATE product SET productImageURL = 'img/3.jpg' WHERE ProductId = @prod3;
UPDATE product SET productImageURL = 'img/4.jpg' WHERE ProductId = @prod4;
UPDATE product SET productImageURL = 'img/5.jpg' WHERE ProductId = @prod5;

-- Loads image data for product 1
UPDATE product SET productImage =  WHERE ProductId = @prod1;
